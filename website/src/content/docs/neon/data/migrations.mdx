---
title: Migrations
description: Apply SQL migrations and seed data to Neon projects and branches as part of every deploy — ordered, hashed, and tracked in Alchemy's migration table.
---
...
`alchemy.__alchemy_migrations` (created automatically), one row per
file:

```sql
CREATE TABLE IF NOT EXISTS "alchemy"."__alchemy_migrations" (
...
skipped. Use `migrations: { dir, table, schema }` to customize its
name or schema:
