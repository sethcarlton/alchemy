...
`__alchemy_migrations`. Postgres stores it in the `alchemy` schema;
D1 and MySQL use their default namespace:
...
Postgres also accepts a custom schema:

```typescript
const db = yield* Neon.Project("app-db", {
  migrations: {
    dir: "./migrations",
    schema: "internal",
  },
...
`alchemy.__alchemy_migrations` on Postgres or `__alchemy_migrations`
elsewhere, and leaves the old table **frozen**:
...
current table shape in place, at whatever location their state
