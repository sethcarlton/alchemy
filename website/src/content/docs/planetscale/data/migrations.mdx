...
Postgres records applied migrations in
`alchemy.__alchemy_migrations`; MySQL uses `__alchemy_migrations` in
the current database. The table is created automatically with one row
per file:
...
is frozen. Postgres accepts `table` and `schema` overrides:
