...
fan-out of messages, [Queues](/cloudflare/messaging/queues). Workflows can
start on a native cron schedule; use [Worker cron triggers](/cloudflare/messaging/cron)
when a Worker's `scheduled` handler should run instead.
...
Pass cron expressions in `schedules` to start Workflow instances without a
...
Cloudflare evaluates schedules in UTC. Pass `schedules: []` to remove every
schedule.

Alchemy records schedules during local development, but its local Workflow
simulator does not fire them automatically. Deploy to Cloudflare to test the
trigger itself.
...
Add `schedules` to this Workflow reference to start instances on native cron
schedules. Add `scriptName` to bind a workflow hosted by another Worker script
— bindings only, so deploy the host first. Cross-script references cannot
declare schedules; the Worker that defines the Workflow owns them. The full
async-handler example lives in the API reference.
...
- [Cron triggers](/cloudflare/messaging/cron) — run a Worker's `scheduled`
  handler.
